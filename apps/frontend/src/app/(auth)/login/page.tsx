...
const GitHubButton = ({ icon }: { icon: React.ReactNode }) => {
  return (
    <button
      type="button"
    ...
    <div className="min-h-screen bg-[#0b0d10] px-4 py-16 text-white">
      <div className="mx-auto w-full max-w-md rounded-2xl border border-white/10 bg-white/[0.03] p-6 shadow-2xl shadow-black/30">
        ...
        <p className="mt-2 text-sm leading-relaxed text-white/60">
          Masuk menggunakan GitHub untuk mengakses Project Library dan Architect Workspace.
        </p>

        <div className="mt-6 space-y-3">
          <GitHubButton icon={<Github className="h-4 w-4" />} />
        </div>
      </div>
    </div>
  );
